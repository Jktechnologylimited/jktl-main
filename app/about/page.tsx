    ...
    <>
      ...
      <section style={{ background:"var(--cream-50)", padding:"80px 32px" }}>
        <div className="max-w-7xl mx-auto px-8">
          <div className="grid md:grid-cols-3 gap-px" style={{ background:"var(--cream-300)" }}>
            {[
              { icon:"🎯", label:"Mission", text:siteConfig.mission },
              { icon:"👁️", label:"Vision",  text:siteConfig.vision  },
              { icon:"💎", label:"Values",  text:siteConfig.values.join(" · ") },
            ].map(item => (
        ...
        <div className="max-w-7xl mx-auto px-8">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-start", gap:"24px" }}>
                ...
                <div style={{ width:"320px", height:"380px", borderRadius:"4px", overflow:"hidden", background:"var(--navy-800)", position:"relative", zIndex:1 }}>
                  {/*
                    ...
                    3. Replace placeholder div with:
                       <Image src="/owner.jpg" alt="Founder JKTL" fill style={{ objectFit:"cover", objectPosition:"top" }} />
                  */}
              ...
              <div>
                {/* Replace "John K." with real founder name */}
            ...
            <div>
              ...
              {[
                "JK Technology Limited was founded in 2019 with one observation that wouldn\'t leave us: great businesses were suffering unnecessarily from poor digital presence. Not because they lacked talent or dedication — but because they had websites without systems.",
                "A website without SEO is invisible. SEO without a lead capture system leaks opportunities. Leads without follow-up automation go cold. Operations without a CRM become chaotic. We saw businesses investing in one piece of the puzzle and wondering why the overall picture wasn\'t coming together.",
                ...
                "Six years and 50+ projects later, we serve businesses across Nigeria, the UK, and beyond — in industries from law and solar energy to auto detailing and financial consulting.",
              ].map((p, i) => <p key={i} className="body-md" style={{ color:"rgba(28,28,30,0.62)", marginBottom:"14px" }}>{p.replace(/\\'/g, "'")}</p>)}
              <div style={{ display:"flex", gap:"12px", flexWrap:"wrap", marginTop:"20px" }}>
                <a href={siteConfig.zoomLink} target="_blank" rel="noopener noreferrer" className="btn-gold">Book a Discovery Call</a>
                <a href={`mailto:${siteConfig.email}`} className="btn-outline-navy">Email Us</a>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {targetAudiences.map(a => (
              <div key={a.title} style={{ padding:"24px", background:"#fff", border:"1px solid var(--cream-300)", borderRadius:"4px" }}>
                <div style={{ fontSize:"1.75rem", marginBottom:"12px" }}>{a.icon}</div>
      ...
      <section style={{ background:"var(--cream-50)", padding:"64px 32px" }}>
        <div className="max-w-7xl mx-auto px-8">
          <div className="mb-8">
            <span className="gold-rule mb-4" style={{ display:"block" }} />
            <h2 className="display-lg" style={{ color:"var(--navy-900)" }}>Serving Businesses Across Nigeria &amp; the UK</h2>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:"8px" }}>
            ...
            <span style={{ fontSize:"0.78rem", color:"rgba(28,28,30,0.6)", background:"rgba(201,168,76,0.08)", padding:"5px 12px", border:"1px solid rgba(201,168,76,0.25)", borderRadius:"2px" }}>United Kingdom</span>
          </div>
        </div>
      </section>
