...
export default function ServicePage() {
  const service = services.find(s => s.slug === "payment-infrastructure");
  if (!service) return null;
  const extra = ["Paystack integration","QR code payments","Recurring billing setup","Automatic receipts"];
  ...
  return (
    <div className="bg-cream-50">
      <section className="bg-navy-950 relative overflow-hidden" style={{ paddingTop:"clamp(88px,12vw,120px)", paddingBottom:"72px" }}>
        ...
        <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-sm mb-6"
            style={{ background:"#05966920", border:"1px solid #05966940" }}>
            <span className="font-mono text-[0.6rem] tracking-widest uppercase font-bold" style={{ color:"#059669" }}>
              Agency Service -- Custom Scoped Work
            </span>
          </div>
          <h1 className="display-hero text-white mb-5 leading-[1.04]">{service.label}</h1>
          <div className="border-l-4 pl-5 mb-6" style={{ borderColor:"#059669" }}>
            <p className="text-white/65 text-[0.95rem] leading-[1.8]" style={{ maxWidth:620 }}>
              If you are still collecting payments by bank transfer and manually confirming them on WhatsApp, you are spending hours every week on admin that should be automated -- and losing sales when the process is too complicated.
            </p>
          </div>
          <div className="p-5 rounded mb-8" style={{ background:"#05966912", border:"1px solid #05966930" }}>
            <p className="font-mono text-[0.6rem] tracking-widest uppercase font-bold mb-2" style={{ color:"#059669" }}>Our Promise</p>
            <p className="text-white font-bold text-[0.95rem] leading-[1.6]">A complete payment system: payment links, QR codes, invoice generation, receipts, and subscription billing -- all connected to your bank account via Paystack.</p>
          </div>
      ...
      <div className="py-3 px-4 text-center" style={{ background:"#059669" }}>
              ...
              {allFeatures.map((f: string) => (
                <div key={f} className="flex gap-3 items-start">
                  <div className="w-6 h-6 rounded-sm flex items-center justify-center shrink-0 mt-0.5"
                    ...
                    <span className="text-[0.65rem] font-bold" style={{ color:"#059669" }}>v</span>
                  </div>
              ...
              <div className="flex flex-col gap-2.5">
                {["24-hour response guarantee","Custom scope -- not a package","Direct founder contact","Nigerian context, Naira pricing"].map((item: string) => (
                  <div key={item} className="flex items-center gap-2.5 text-[0.82rem] text-black/65">
                    <span style={{ color:"#059669" }}>v</span>{item}
                  </div>
                ))}
              </div>
            </div>
