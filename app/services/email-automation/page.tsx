...
export default function ServicePage() {
  const service = services.find(s => s.slug === "email-automation");
  if (!service) return null;
  const extra = ["Resend email integration","Domain email setup","Lead nurture sequences","Payment confirmation emails"];
  ...
  return (
    <div className="bg-cream-50">
      <section className="bg-navy-950 relative overflow-hidden" style={{ paddingTop:"clamp(88px,12vw,120px)", paddingBottom:"72px" }}>
        <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background:"#7C3AED" }} />
        <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-sm mb-6"
            ...
            <span className="font-mono text-[0.6rem] tracking-widest uppercase font-bold" style={{ color:"#7C3AED" }}>
              Agency Service -- Custom Scoped Work
            </span>
          </div>
          <h1 className="display-hero text-white mb-5 leading-[1.04]">{service.label}</h1>
          <div className="border-l-4 pl-5 mb-6" style={{ borderColor:"#7C3AED" }}>
            <p className="text-white/65 text-[0.95rem] leading-[1.8]" style={{ maxWidth:620 }}>
              Every lead that contacts your business and does not hear back within 5 minutes is 80% less likely to convert. Most Nigerian businesses lose these leads every single day.
            </p>
          </div>
          <div className="p-5 rounded mb-8" style={{ background:"#7C3AED12", border:"1px solid #7C3AED30" }}>
            <p className="font-mono text-[0.6rem] tracking-widest uppercase font-bold mb-2" style={{ color:"#7C3AED" }}>Our Promise</p>
            <p className="text-white font-bold text-[0.95rem] leading-[1.6]">Automated email sequences that follow up with every lead, confirm every booking, and recover every missed opportunity -- without you lifting a finger.</p>
          </div>
      ...
      <div className="py-3 px-4 text-center" style={{ background:"#7C3AED" }}>
              ...
              {allFeatures.map((f: string) => (
                <div key={f} className="flex gap-3 items-start">
                  <div className="w-6 h-6 rounded-sm flex items-center justify-center shrink-0 mt-0.5"
                    ...
                    <span className="text-[0.65rem] font-bold" style={{ color:"#7C3AED" }}>v</span>
                  </div>
              ...
              <div className="flex flex-col gap-2.5">
                {["24-hour response guarantee","Custom scope -- not a package","Direct founder contact","Nigerian context, Naira pricing"].map((item: string) => (
                  <div key={item} className="flex items-center gap-2.5 text-[0.82rem] text-black/65">
                    <span style={{ color:"#7C3AED" }}>v</span>{item}
                  </div>
                ))}
              </div>
            </div>
