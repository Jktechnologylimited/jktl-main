...
export default function ServicePage() {
  const service = services.find(s => s.slug === "lead-generation");
  if (!service) return null;
  const extra = ["Meta and Google Ads integration","Lead capture with CRM sync","Conversion tracking setup","Full sales copy included"];
  ...
  return (
    <div className="bg-cream-50">
      <section className="bg-navy-950 relative overflow-hidden" style={{ paddingTop:"clamp(88px,12vw,120px)", paddingBottom:"72px" }}>
        <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background:"#1A6E3C" }} />
        <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-sm mb-6"
            style={{ background:"#1A6E3C20", border:"1px solid #1A6E3C40" }}>
            <span className="font-mono text-[0.6rem] tracking-widest uppercase font-bold" style={{ color:"#1A6E3C" }}>
              Agency Service -- Custom Scoped Work
            </span>
          </div>
          <h1 className="display-hero text-white mb-5 leading-[1.04]">{service.label}</h1>
          <div className="border-l-4 pl-5 mb-6" style={{ borderColor:"#1A6E3C" }}>
            <p className="text-white/65 text-[0.95rem] leading-[1.8]" style={{ maxWidth:620 }}>
              Every naira you spend on ads is wasted if the page you send traffic to does not convert. Most Nigerian businesses are burning ad budget on pages that were never designed to sell.
            </p>
          </div>
          <div className="p-5 rounded mb-8" style={{ background:"#1A6E3C12", border:"1px solid #1A6E3C30" }}>
            <p className="font-mono text-[0.6rem] tracking-widest uppercase font-bold mb-2" style={{ color:"#1A6E3C" }}>Our Promise</p>
            <p className="text-white font-bold text-[0.95rem] leading-[1.6]">A dedicated landing page that converts ad traffic into leads at the highest possible rate -- with A/B testing, tracking, and copy included.</p>
          </div>
      ...
      <div className="py-3 px-4 text-center" style={{ background:"#1A6E3C" }}>
              ...
              {allFeatures.map((f: string) => (
                <div key={f} className="flex gap-3 items-start">
                  <div className="w-6 h-6 rounded-sm flex items-center justify-center shrink-0 mt-0.5"
                    style={{ background:"#1A6E3C15" }}>
                    <span className="text-[0.65rem] font-bold" style={{ color:"#1A6E3C" }}>v</span>
                  </div>
              ...
              <div className="flex flex-col gap-2.5">
                {["24-hour response guarantee","Custom scope -- not a package","Direct founder contact","Nigerian context, Naira pricing"].map((item: string) => (
                  <div key={item} className="flex items-center gap-2.5 text-[0.82rem] text-black/65">
                    <span style={{ color:"#1A6E3C" }}>v</span>{item}
                  </div>
                ))}
              </div>
            </div>
