...
export default function ServicePage() {
  ...
  const extra = ["WhatsApp auto-reply setup","Custom AI training","Lead qualification flow","Human handoff trigger"];
  ...
  return (
    <div className="bg-cream-50">
      <section className="bg-navy-950 relative overflow-hidden" style={{ paddingTop:"clamp(88px,12vw,120px)", paddingBottom:"72px" }}>
        <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background:"#0F172A" }} />
        <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-sm mb-6"
            style={{ background:"#0F172A20", border:"1px solid #0F172A40" }}>
            <span className="font-mono text-[0.6rem] tracking-widest uppercase font-bold" style={{ color:"#0F172A" }}>
              Agency Service -- Custom Scoped Work
            </span>
          </div>
          <h1 className="display-hero text-white mb-5 leading-[1.04]">{service.label}</h1>
          <div className="border-l-4 pl-5 mb-6" style={{ borderColor:"#0F172A" }}>
            <p className="text-white/65 text-[0.95rem] leading-[1.8]" style={{ maxWidth:620 }}>
              While you sleep, your competitors with AI chatbots are qualifying leads, answering questions, and booking appointments automatically. Every hour your business is not answering is an hour your competitor is.
            </p>
          </div>
          <div className="p-5 rounded mb-8" style={{ background:"#0F172A12", border:"1px solid #0F172A30" }}>
            <p className="font-mono text-[0.6rem] tracking-widest uppercase font-bold mb-2" style={{ color:"#0F172A" }}>Our Promise</p>
            <p className="text-white font-bold text-[0.95rem] leading-[1.6]">An AI assistant trained on your business that qualifies leads, answers questions, books appointments, and hands off to your team -- 24 hours a day.</p>
          </div>
      ...
      <div className="py-3 px-4 text-center" style={{ background:"#0F172A" }}>
              ...
              {allFeatures.map((f: string) => (
                <div key={f} className="flex gap-3 items-start">
                  <div className="w-6 h-6 rounded-sm flex items-center justify-center shrink-0 mt-0.5"
                    style={{ background:"#0F172A15" }}>
                    <span className="text-[0.65rem] font-bold" style={{ color:"#0F172A" }}>v</span>
                  </div>
              ...
              <div className="flex flex-col gap-2.5">
                {["24-hour response guarantee","Custom scope -- not a package","Direct founder contact","Nigerian context, Naira pricing"].map((item: string) => (
                  <div key={item} className="flex items-center gap-2.5 text-[0.82rem] text-black/65">
                    <span style={{ color:"#0F172A" }}>v</span>{item}
                  </div>
                ))}
              </div>
            </div>
