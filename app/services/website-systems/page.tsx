...
export default function ServicePage() {
  const service = services.find(s => s.slug === "website-systems");
  if (!service) return null;
  const extra = ["SEO-optimised copy","WhatsApp integration","Google Maps embed"];
  ...
  return (
    <div className="bg-cream-50">
      <section className="bg-navy-950 relative overflow-hidden" style={{ paddingTop:"clamp(88px,12vw,120px)", paddingBottom:"72px" }}>
        <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background:"#1A4A8A" }} />
        <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-sm mb-6"
            style={{ background:"#1A4A8A20", border:"1px solid #1A4A8A40" }}>
            <span className="font-mono text-[0.6rem] tracking-widest uppercase font-bold" style={{ color:"#1A4A8A" }}>
              Agency Service -- Custom Scoped Work
            </span>
          </div>
          <h1 className="display-hero text-white mb-5 leading-[1.04]">{service.label}</h1>
          <div className="border-l-4 pl-5 mb-6" style={{ borderColor:"#1A4A8A" }}>
            <p className="text-white/65 text-[0.95rem] leading-[1.8]" style={{ maxWidth:620 }}>
              Your competitors have websites that rank on Google and capture leads automatically. If yours does not, you are invisible to the customers searching for your service right now.
            </p>
          </div>
          <div className="p-5 rounded mb-8" style={{ background:"#1A4A8A12", border:"1px solid #1A4A8A30" }}>
            <p className="font-mono text-[0.6rem] tracking-widest uppercase font-bold mb-2" style={{ color:"#1A4A8A" }}>Our Promise</p>
            <p className="text-white font-bold text-[0.95rem] leading-[1.6]">A professional business website that ranks on Google, captures leads, and represents your business at the highest level -- from day one.</p>
          </div>
      ...
      <div className="py-3 px-4 text-center" style={{ background:"#1A4A8A" }}>
              ...
              {allFeatures.map((f: string) => (
                <div key={f} className="flex gap-3 items-start">
                  <div className="w-6 h-6 rounded-sm flex items-center justify-center shrink-0 mt-0.5"
                    style={{ background:"#1A4A8A15" }}>
                    <span className="text-[0.65rem] font-bold" style={{ color:"#1A4A8A" }}>v</span>
                  </div>
              ...
              <div className="flex flex-col gap-2.5">
                {["24-hour response guarantee","Custom scope -- not a package","Direct founder contact","Nigerian context, Naira pricing"].map((item: string) => (
                  <div key={item} className="flex items-center gap-2.5 text-[0.82rem] text-black/65">
                    <span style={{ color:"#1A4A8A" }}>v</span>{item}
                  </div>
                ))}
              </div>
            </div>
